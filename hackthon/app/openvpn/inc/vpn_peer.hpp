#ifndef __vpn_peer_hpp__
#define __vpn_peer_hpp__
...
//
// ═══════════════════════════════════════════════════════════════════════════
// Tunnel frame format  (TCP stream between vpn_server ↔ vpn_client)
// ═══════════════════════════════════════════════════════════════════════════
//
//   ┌────────┬────────────────────────┬──────────────────────────────────┐
...
//   → vpn_peer receives, parses rpc_path '\0' proto_bytes
...
public:
  ...
  vpn_peer(int32_t channel, const std::string &peer_host,
               vpn_server *parent, const std::string &assigned_ip,
               const std::string &netmask, const mqtt_sub_cfg &mqtt = {});

  // TLS or pre-wrapped bev peer.
  vpn_peer(struct bufferevent *bev, const std::string &peer_host,
               vpn_server *parent, const std::string &assigned_ip,
               const std::string &netmask, const mqtt_sub_cfg &mqtt = {});

  virtual ~vpn_peer() = default;
